...
Component({
		...
		todayTasksShow(data) {
			...
			let res = data.tasks.filter(function (item) {
				return (
					...
					(item.list.title === data.pageName || data.pageName === "运动")
				);
			});
			res.sort((a, b) =>
	...
	watch: {
		selectIconRotate: function (selectIconRotate) {
			...
			let height = Math.min(8 * 36, 36 * (this.data.types.length + 1) - 2);
			this.animate(
				".listSelectWrap",
				[
	...
	methods: {
		...
		handleSelectList: function (e) {
			...
			let pageName = index == undefined ? "全部" : this.data.types[index];
